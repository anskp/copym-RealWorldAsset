...
const isIssuer = async (req, res, next) => {
  try {
    ...
    console.log('User in isIssuer middleware:', userId);
    
    // Check for userrole in the user object
    let hasIssuerRole = false;
    
    if (req.user.userrole && Array.isArray(req.user.userrole)) {
      hasIssuerRole = req.user.userrole.some(r => r.role === 'ISSUER');
    } else {
      // Fetch roles from database if not available in user object
      const userRoles = await prisma.userrole.findMany({
        where: { user_id: userId }
      });
      
      hasIssuerRole = userRoles.some(r => r.role === 'ISSUER');
      
      // Add roles to user object for future use
      req.user.userrole = userRoles;
    }
    
    if (!hasIssuerRole) {
      console.error(`User ${userId} is not an issuer`);
      return res.status(403).json({ 
        success: false, 
        message: 'Forbidden - Issuer access required' 
      });
    }
    
    console.log(`User ${userId} verified as issuer`);
    next();
  } catch (error) {
