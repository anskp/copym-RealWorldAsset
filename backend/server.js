...
const fireblocksRoutes = require('./routes/fireblocks-routes');
...
const fireblocksVaultsRouter = require('./routes/fireblocks/vaults.routes');
...
app.use('/api/fireblocks', fireblocksRoutes);
...
// Home route
app.get('/', (req, res) => {
  const mockEnabled = process.env.USE_MOCK_FIREBLOCKS === 'true';
  ...
  res.json({
    ...
    endpoints: {
      create_vault: '/api/fireblocks/vault',
      get_vault: '/api/fireblocks/vault/:id',
      rename_vault: '/api/fireblocks/vault/:vaultId',
      create_wallet: '/api/fireblocks/vault/:vaultId/:assetId/wallet',
      activate_wallet: '/api/fireblocks/vault/:vaultId/:assetId/activate',
      get_balance: '/api/fireblocks/vault/:vaultId/:assetId/balance',
      refresh_balance: '/api/fireblocks/vault/:vaultId/:assetId/balance/refresh'
    }
  });
});
...
async function initializeFireblocks() {
  try {
    ...
    await fireblocksService.initialized;
    console.log('Fireblocks service initialized');
    console.log('Using mock mode:', fireblocksService.useMockMode ? 'Yes' : 'No');
    return fireblocksService;
  } catch (error) {
    console.error('Failed to initialize Fireblocks service:', error.message);
    console.log('Falling back to mock mode');
    return null;
  }
}

// Health check endpoint for Fireblocks
app.get('/health/fireblocks', async (req, res) => {
  try {
    const fireblocksService = require('./services/fireblocks.service');
    res.status(200).json({
      status: 'UP',
      ...
      services: {
        fireblocks: {
          status: fireblocksService.useMockMode ? 'MOCK' : 'LIVE',
          baseUrl: fireblocksService.baseUrl,
          initialized: fireblocksService.initialized ? true : false
        }
      }
    });
  } catch (error) {
...
app.listen(PORT, async () => {
  ...
  console.log(`Health check available at http://localhost:${PORT}/health/fireblocks`);
  console.log(`Mock mode: ${process.env.USE_MOCK_FIREBLOCKS === 'true' ? 'ENABLED' : 'DISABLED'}`);
  console.log(`API URL: ${process.env.FIREBLOCKS_BASE_URL || 'https://sandbox-api.fireblocks.io'}`);
  
  // Initialize Fireblocks service
  await initializeFireblocks();
});
